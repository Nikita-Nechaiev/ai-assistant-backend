...
export class RolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const sessionId = client.handshake.query.sessionId || client.handshake.headers.sessionid;

    if (!userId || !sessionId) {
      throw new ForbiddenException('User or session ID not found');
    }

    const userSession = await this.userCollaborationSessionService.findByUserAndSession(userId, Number(sessionId));

    if (!userSession) {
      throw new ForbiddenException('User is not part of this session');
    }

    const hasPermission = requiredPermissions.some((perm) => userSession.permissions.includes(perm));
